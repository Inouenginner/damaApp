...
export const fetchWazas = (name) => {
    return async (dispatch, getState) => {
        ...
        console.log(detailsDocs.docs.map(detailsDoc => detailsDoc.memo))
        detailsDocs.forEach((detailsDoc) => {
            console.log(detailsDoc.memo, ' => ', JSON.stringify(detailsDoc.data()))
          })
        if (detailsDocs.exists) {
            console.log("Document data:", detailsDocs.data());
        } else {
...
export const signIn = (name) => {
    return async (dispatch, getState) => {
        ...
        if (userDoc.exists) {
            ...
            const detailsRef = db.collection('details').doc(name).set({ name: name,});
