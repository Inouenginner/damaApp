    ...
    return async (dispatch, getState) => {
        ...
        db.collection("users").where("name", "==", name).get().then(snapShot => {
            snapShot.forEach(doc => {
              const sameIdUserData = doc.data()
              console.log(sameIdUserData)
            })
          })
       // const userDoc = await db.collection('users').where("name", "==", name).get();
       // console.log(userDoc.data());
        let id;
        //すでにuserがあるのならば
        if (sameIdUserData.exists) {
            id = sameIdUserData.data()["id"];
            console.log("already exist user");
        } else {
            //db1に挿入
            //idは連番にしたい
            db.collection("users").orderBy("id","asc").limit(1).get().then(snapShot => {
                snapShot.forEach(doc => {
                  const data = doc.data()
                  console.log(data)
                })
            ...
            //const userDocOrderById = await db.collection('users').orderBy("id","asc").limit(1).get();
            console.log(userDocOrderById.data())
