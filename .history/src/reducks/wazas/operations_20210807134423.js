...
export const signIn = (name) => {
    return async (dispatch, getState) => {
        ...
        let wazas = [];
            ...
            .then(res => {
                console.log("うんち"+JSON.stringify(res.data))
                console.log("うんち"+JSON.stringify(res.data[0]))
                wazas.push(res.data[0]);
            })
            console.log("うんち"+JSON.stringify(wazas))
            console.log("うんち"+JSON.stringify(wazas[0]))
        //valid
        if(name === "") {
            alert("必須項目が未入力です")
            return false
