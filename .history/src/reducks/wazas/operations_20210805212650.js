    ...
    return async (dispatch, getState) => {
        ...
        if (sameIdUserData != null) {
            ...
            for (let num = 1; num <= 200; num++) {
                let details1Ref = db.collection('details').doc(id).collection('results').doc(String(num));
                batch.set(details1Ref, { id:num, achieve: "", memo: "", favorite: false, makeDay: timestamp});
            }
            // バッチ処理の完了を宣言
            await batch.commit();
        }
