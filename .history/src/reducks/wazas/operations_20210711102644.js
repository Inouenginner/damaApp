...
export const signIn = (name) => {
    return async (dispatch, getState) => {
        ...
        const detailsRef = db.collection('wazas').doc(name).set({ });
        const details1Ref = db.collection('wazas').doc(name).collection('results').doc('1');
        await details1Ref.set({ id: "1", favoiteSum: "", level: false, makeSum: new Date('2019-02-08 00:00:00'), url:"https://www.youtube.com/embed/cWDJoK8zw58",　waza:"世界一周"})
