...
export const updateDetail = (achieve,memo,favorite, wazaId, userId) => {
    return async (dispatch, getState) => {
        ...
        //DBへの登録
        const detailsRef = db.collection('details').doc(String.valueOf(userId)).collection('results').doc(wazaId);
...
//         await batch.commit();
        ...
        if (sameIdUserData != null) {
            ...
            const timestamp = FirebaseTimestamp.now()
            let batch = db.batch();
            const userStandardRef = db.collection('users').doc(String.valueOf(userId));
            ...
            const detailsRef = db.collection('details').doc(String.valueOf(userId))
            batch.set(detailsRef, {});
            const wazaSum = 200;
            let num = 1;
            while (num <= wazaSum) {
                let detailsRef = db.collection('details').doc(String.valueOf(userId)).collection('results').doc(String.valueOf(num));
        ...
        const wazasDoc = await wazasRef.doc(String.valueOf(userId)).get()
        ...
        const detailsRef = db.collection('details').doc(String.valueOf(userId)).collection('results');
        ...
        detailsDocsOrderById.forEach((detailsDocOrderById) => {
            if (detailsDocOrderById.exists) {
                joinedList.push(Object.assign(userWazaList[detailsDocOrderById.data()["id"] - 1], detailsDoc.data()));
            }
        });
