...
export const fetchWazas = (name) => {
    return async (dispatch, getState) => {
        ...
        const detailsRef = db.collection('details').doc(name);
        const detailsDoc = await detailsRef.get() // firebase.firestore.DocumentSnapshotのインスタンスを取得
        console.log(detailsRef);
        console.log(detailsDoc);
        detailsDoc.get().then((doc) => {
            if (doc.exists) {
        ...
        if (detailsDoc.exists) {
            console.log(detailsDoc.data())
          } else {
            console.log('No such doc　ument!')
          }
